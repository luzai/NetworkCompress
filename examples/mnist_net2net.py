...
def wider2net_conv2d(teacher_w1, teacher_b1, teacher_w2, new_width, init):
    ...
    '''
    # keras1: (output_channels,input_channels,width,height) keras2:(width,height,input_channels,output_channels)
    # 0 1 2 3 --> 3 2 0 1
...
def deeper2net_conv2d(teacher_w):
    ...
    # Arguments
        teacher_w: `weight` of previous conv2d layer,
          of shape (filters, num_channel, kh, kw) -- keras1 
          (kw,kh,num_channel,filters) -- keras2
    '''
...
def copy_weights(teacher_model, student_model, layer_names):
    ...
    for name in layer_names:
        weights = teacher_model.get_nodes(name=name).get_weights()
        if student_model.get_nodes(name=name) is not None:
            ...
            ind = names2ind[name]
            student_model.layers[ind].set_weights(weights)
...
def make_wider_student_model(teacher_model, train_data,
    ...
    w_conv1, b_conv1 = teacher_model.get_nodes('conv1').get_weights()
    w_conv2, b_conv2 = teacher_model.get_nodes('conv2').get_weights()
    ...
    w_fc1, b_fc1 = teacher_model.get_nodes('fc1').get_weights()
    w_fc2, b_fc2 = teacher_model.get_nodes('fc2').get_weights()
